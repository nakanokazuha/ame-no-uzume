...
import stopEnvelope from "../fake_hermes/fixtures/hook-stop.json";
...
test("enhanced telemetry shows verified role and goal", async ({ page, request }) => {
  ...
  await postHook(request, startEnvelope);
  ...
  await postHook(request, stopEnvelope);
  await expect(page.getByText("Researcher")).not.toBeVisible();
